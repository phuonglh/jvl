...
# ╔═╡ ca30b078-fe40-11eb-0b74-593f58046403
using ImageView

# ╔═╡ 9f6f14c0-8103-40f5-a4ec-5edd5dfefff8
using Images

# ╔═╡ 3051a544-0885-43ce-91c0-0a90b98a558a
using MLDatasets

# ╔═╡ ac8908eb-012e-4834-ab22-dd835c260af1
using Flux

# ╔═╡ 67261483-21bf-40cb-a9b7-596bcfc5a63d
using PlutoUI

# ╔═╡ 2a8f2ad9-28d4-4f20-b714-c2de93352b2e
begin
	X_train, y_train = MNIST.traindata()
	X_test, y_test = MNIST.testdata()
end

# ╔═╡ 384cb2e4-20d2-48f6-9510-505f72c4e6c3
size(X_test)

# ╔═╡ ca700172-5a21-49be-84b0-5dffc1ae5041
size(y_test)

# ╔═╡ 0601a577-f395-4e01-918b-ab31462b4e85
size(X_train)

# ╔═╡ f6876b33-0146-4791-8387-483040fa7590
x1 = X_train[:,:,2]

# ╔═╡ 2dba738f-5037-4dbf-853c-479b1ee0f364
imshow(x1)

# ╔═╡ 546bd532-f7b0-4bcf-be7f-f14ca82eed0e
begin
	struct BWImage
		data::Array{UInt8, 2}
		zoom::Int
...
# ╔═╡ 3fb94107-e8f0-44a4-b3fa-81b17cb0e3a4
BWImage(x1, zoom=10) # use this function to show the image directly in Pluto

# ╔═╡ 5034a3a6-489d-46b0-b78d-2655fa9bdcbc
y_train[1]

# ╔═╡ 74989b05-9b9c-444b-a517-c093bb2c3c9f
unique(y_test)

# ╔═╡ 50d1e117-61a4-491c-95bd-d64dda87e96d
layer1 = Dense(28*28, 64)

# ╔═╡ dc02e144-3d19-44ac-a7d5-0a3a93bb42d7
a1 = layer1(vec(x1))

# ╔═╡ 33b86741-9b75-4312-a263-6b4eda2fd449
layer2 = Dense(64, 10)

# ╔═╡ 2c3f988e-f104-4fc6-b84d-6246517f55e7
a2 = layer2(a1)

# ╔═╡ 07954fa2-6a47-4ce5-9fb0-17f35739057c
a = softmax(a2)

# ╔═╡ 9cfb3900-f11b-4cbc-9e40-a92e8f541611
model = Chain(layer1, layer2)

# ╔═╡ 67c6be2e-7dd9-4724-86e6-c5962d85345e
model(vec(x1))

# ╔═╡ 1949b4c4-8974-46e5-bcc4-ceac72730788
y1 = y_test[1]

# ╔═╡ bb5e5236-c4d5-44b7-ae99-575bef4c8e44
z = Flux.onehot(y1, 0:9)

# ╔═╡ d719d435-1d21-40b0-ae77-9041260278e0
Flux.logitcrossentropy(a, z)

# ╔═╡ 1c67466a-7960-41ef-a946-b702a2a9e44c
Flux.crossentropy(z, z)

# ╔═╡ 4d9d6b97-1d20-4bbf-9d71-d516d701ddf4
Flux.logitcrossentropy(a, a)

# ╔═╡ 188e3667-c428-4f3d-aa6e-c94f545a4fef
loader_x = Flux.Data.DataLoader(X_test, batchsize=64)

# ╔═╡ 9a47332e-fa70-4821-95f4-12bf1e18986c
first(loader_x)

# ╔═╡ ea41b836-2a87-4f72-ad63-527b5631c1ad
vectorizeBatch(X) = Flux.batch([Float32.(vec(X[:,:,i])) for i=1:size(X,3)])

# ╔═╡ 064b6909-017c-48ae-abf4-08c971113cd8
vectorizeBatch(first(loader_x))

# ╔═╡ a2eeabea-0815-43c3-b962-57acd3618e5a
size(first(loader_x))

# ╔═╡ bc51cd3b-fa7a-4738-a3b0-60217ca1cf92
loader_y = Flux.Data.DataLoader(y_test, batchsize=64)

# ╔═╡ 1ee64a4d-e26d-4954-af23-8105d5ee2125
first(loader_y)

# ╔═╡ ea54246d-17e8-455c-a7d5-81e480c774b2
Xbs = [vectorizeBatch(Xb) for Xb in loader_x]

# ╔═╡ 163bd4ef-ab90-4a05-a8f1-ee55b0b96436
length(Xbs)

# ╔═╡ 1b31a8ad-6248-422d-b4c1-23b1700c8732
157*64

# ╔═╡ 42d8530f-efc8-4469-a96d-5b3400577b1b
ybs = [Flux.onehotbatch(y, 0:9) for y in loader_y]

# ╔═╡ d0aa6e2b-2f61-4ffb-a8f0-eb919f5c4125
length(ybs)

# ╔═╡ 91806a42-d36e-41b5-a5d8-57f2eae617e6
loss(Xb, yb) = Flux.logitcrossentropy(model(Xb), yb) 

# ╔═╡ 6c1c5024-988b-42fa-9a14-76b031d7c362
evalcb() = @show(sum(loss(Xbs[1], ybs[1])))

# ╔═╡ ca1f29a5-e76e-45f9-9283-4db819a81f63
optimizer = ADAM()

# ╔═╡ 64958b53-7bc1-4464-b987-c773a21a46ea
PlutoUI.with_terminal() do
	Flux.@epochs 50 Flux.train!(loss, params(model), zip(Xbs, ybs), optimizer, cb = Flux.throttle(evalcb, 10))
end

# ╔═╡ 6cc31f18-a736-4729-9191-2fbd4935f387
model[1].weight

# ╔═╡ 6ce791bc-9877-4f32-9d63-c44154be1025
model[1].bias

# ╔═╡ cab113fe-f497-4b9e-b25f-bfbc318230f8
function classify(Xb)
	a = model(Xb)
	Flux.onecold(a)
end

# ╔═╡ 59322e44-6645-42ae-be70-2f6c142be34d
classify(Xbs[1])

# ╔═╡ d4c99c13-c982-49d7-8214-1ccdbc3d1a35
prediction = [classify(Xb) for Xb in Xbs]

# ╔═╡ 07064436-a4f3-40e7-af80-bc276fda1e1f
ŷ = collect(Iterators.flatten(prediction))

# ╔═╡ 241b2966-86b2-41d4-95ba-b87bb846e780
sum(ŷ .== y_test)/length(y_test)*100
