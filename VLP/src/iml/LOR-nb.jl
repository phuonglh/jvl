...
# ╔═╡ b9fe7c13-3493-4e4f-97f8-13d81df1a43c
using DelimitedFiles

# ╔═╡ 65ac64a5-c539-458d-966c-efd8b4b5625d
using Statistics

# ╔═╡ 3802638f-b016-4c0a-b405-fe5cc7adcd32
using Plots

# ╔═╡ ad66dd8d-c12a-420b-bded-51ece5f7df47
function normalize(X)
	μ = mean(X, dims=1)
	σ = std(X, dims=1)
	M = repeat(μ, size(X,1), 1)
	Σ = repeat(σ, size(X,1), 1)
	return (X - M) ./ Σ
end

# ╔═╡ ad6f0768-f5b8-11eb-11b6-0de5f4bd1915
function readData(path)
	A = readdlm(path, ',')
	y = A[:,2]
	X_norm = normalize(A[:,3:end])
	X = hcat(ones(length(y)), X_norm)
	return (X, y)
end

# ╔═╡ a4128566-f44e-4a02-83c9-26938ac70d58
X, y = readData("/home/phuonglh/courses/iml/dat/wdbc.txt")

# ╔═╡ 145a83e1-61ad-408a-a877-ec09a5630805
g(z) = 1 ./ (1 .+ exp.(-z))

# ╔═╡ 16e9b92b-b148-4bbe-b839-709fd897cb33
function train(X, y, θ_0, T=100, α=0.01)
	θ = θ_0
	θs = []
	for t=1:T
		∇J = X'*(g(X*θ) - y)
...
# ╔═╡ 5566362c-31bd-46fe-9036-ce7f9d15080b
begin
	D = size(X,2)
	θ_0 = zeros(D)
	θs = train(X, y, θ_0, 1000, 0.01)
end

# ╔═╡ bfa71e60-6efc-47ba-98a8-8ed762db7cb4
function J(X, y, θ, ϵ=1E-4)
	h = g(X*θ)
	a = y'*log.(h .+ ϵ)
	b = (1 .- y)'*log.(1 .- h .+ ϵ)
	N = length(y)
...
# ╔═╡ d2ac4539-52eb-4db5-944e-4575117209c2
Js = [J(X, y, θ) for θ ∈ θs]

# ╔═╡ e5922fce-aa97-41b0-a3a1-401b3f9d5a8b
J(X, y, θs[1])

# ╔═╡ b2100f9d-e8ed-4f80-9a83-b7be82a22e83
plot(1:length(Js), Js, xlabel="iterations", ylabel="J", legend=false)

# ╔═╡ d6ca25fd-f30c-451f-a0f0-d2a718f52c86
(Js[100], Js[500], Js[end])

# ╔═╡ 72a324ed-168e-4721-8e80-949f7d0b9d0d
function classify(x, θ)
	return g(x'*θ) < 0.5 ? 0 : 1
end

# ╔═╡ 199e90a6-c12e-43f4-89fa-967bcac2ac06
θ_best = θs[end]

# ╔═╡ b1a9f21c-2032-428d-b189-317c23a2bb3c
classify(X[1,:],θ_best)

# ╔═╡ 5bc4454d-4bff-4e79-8a12-60e448dc4fc5
function evaluate(X, y, θ)
	N = length(y)
	z = [classify(X[i,:], θ) for i=1:N]
	return sum(z .== y)/N * 100
end

# ╔═╡ 194262a8-7c40-4aed-ae8d-46e9259874c9
evaluate(X, y, θ_best)

# ╔═╡ da9059cd-7ff9-4838-a361-e6028dda6343
