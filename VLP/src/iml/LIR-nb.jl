...
# ╔═╡ 77623433-05bc-4829-a0d8-494cedddc4bb
using DelimitedFiles

# ╔═╡ 58af6a55-9c35-421a-9e14-2650186a36d3
using Plots

# ╔═╡ 71687752-f358-11eb-036b-fff84b77e05e
...
# ╔═╡ e115307e-7f67-4952-985d-fd078b94520f
X, y = readData("/home/phuonglh/courses/aml/dat/wdbc.txt")

# ╔═╡ da5e95c4-c40e-487c-a947-5778058eebff
md" $y = f(x) := \langle x, \theta \rangle, y \in \mathbb R$" 

# ╔═╡ 168a3809-366b-4b6f-b074-0369a31e9b0f
function train(X, y)
	return inv(X'*X)*X'*y # normal equation of the linear regression model
end

# ╔═╡ c85b9d4a-6ed6-4ee4-840b-924a84e44f46
θ̂ = train(X, y)

# ╔═╡ 287506a9-46d8-4168-9b65-6c668e089890
function predict(x, θ̂)
	return x'*θ̂
end

# ╔═╡ d90065c7-c95c-48e6-a55c-867decad7062
predict(X[1,:], θ̂)

# ╔═╡ 160190d0-6176-4732-a797-1455ee263473
predict(X[end,:], θ̂)

# ╔═╡ b1cfe2b1-8e95-4d35-b433-170b1c8a19e1
function classify(x::Array{Float64,1}, θ̂, α=0.5)
	score = predict(x, θ̂)
	if (score > α)
		return 1.0 
	else 
...
# ╔═╡ c135a823-6933-4dc5-b494-c8952ad93ea2
function classify(X::Array{Float64,2}, θ̂, α=0.5)
	return [classify(X[i,:], θ̂, α) for i=1:size(X,1)]
end

# ╔═╡ 1490ae16-dddb-4bc9-a18a-8ba1665c9e26
first = classify(X[1,:], θ̂)

# ╔═╡ 9db339aa-6dd4-4e4f-92a8-423ab6670a60
last = classify(X[end,:], θ̂)

# ╔═╡ d5732e01-753e-44c5-9656-7063707c1a90
function evaluate(X, y, θ̂, α=0.5)
	z = classify(X, θ̂, α)
	return sum(z .== y)/length(y) * 100
end

# ╔═╡ 5a1b4d56-2ede-4ed1-85ed-fda8cb2917a4
trainingAccuracy = evaluate(X, y, θ̂)

# ╔═╡ ad8aabde-47c4-47e4-815b-4ea7686ef87f
evaluate(X, y, θ̂, 0.4)

# ╔═╡ 9680af1f-d46f-4b52-8802-1c64dc1d5490
evaluate(X, y, θ̂, 0.6)

# ╔═╡ ac284210-9643-41c6-ab49-b5217cd2a23f
αs = 0.4:0.01:0.8

# ╔═╡ 567ae689-3fc5-4a98-81d2-df0332abb86a
collect(αs)

# ╔═╡ 8008d253-d531-4240-94ff-7c3722ce771c
accuracies = [evaluate(X, y, θ̂, α) for α ∈ αs]

# ╔═╡ a27af3cc-f8cd-4f8d-9ef7-ffb88f2ae6ff
plot(αs, accuracies, xlabel="threshold α", ylabel="accuracy (%)", label=false, title="Training Accuracy")

# ╔═╡ 00fbcc85-5c88-467f-934f-54047cb248cc
best_alpha = αs[argmax(accuracies)]

# ╔═╡ a2d1b512-8c2d-4b34-ba67-b3f0bbfdc413
