module ArcEagerParserEx

export TransitionClassifierEx

using FLoops
using Flux
using DataStructures

include("ClassifierEx.jl")

using .TransitionClassifierEx
using .TransitionClassifierEx.Corpus
import .TransitionClassifierEx.Arc
import .TransitionClassifierEx.Config
import .TransitionClassifierEx.next
import .TransitionClassifierEx.vectorizeSentence
...
function run(mlp, wordIndex, shapeIndex, posIndex, labelIndex, options::Dict{Symbol,Any}, sentences::Array{Sentence})
    ...
    @floop ThreadedEx(basesize=length(sentences)÷options[:numCores]) for sentence in sentences
        ...
        while !isempty(β) && !isempty(σ)
            ...
            cs = map(word -> get(wordIndex, lowercase(word), wordIndex[options[:unknown]]), fs)
            x = (as, bs, cs) # input to our model is a triple of (matrix, vector, vector)
            y = mlp(x)
