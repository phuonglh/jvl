...
bricks = Brick[
    ...
    Brick("date4", raw"\b([Jj]anuary|[Ff]ebruary|[Mm]arch|[Aa]pril|[Mm]ay|[Jj]une|[Jj]uly|[Aa]ugust|[Ss]eptember|[Oo]ctober|[Nn]ovember|[Dd]ecember)[\s_]+\d{1,2}(st|nd|rd|th)?\b", 1),
    Brick("order", raw"\b\d+(st|nd|rd|th)\b", 1),
    Brick("compound", raw"\b\w+\-\w+\b", 1),
    Brick("time", raw"\b(\d{1,2}:\d{1,2}|\d{1,2}(pm|PM|am|AM))\b", 1),
    Brick("numberSeq", raw"\+?\d+(([\s.-]+\d+)){2,}\b", 1),
    Brick("duration", raw"\b\d{4}\-\d{4}\b", 1),
    Brick("currency", raw"\p{Sc}+\s?(\d*)?\d+([.,]\d+)*\b"),
    Brick("number", raw"\b([+-]?(\d*)?[\d]+([.,]\d+)*%?)\b"),
    Brick("item", raw"\d+[.)]\b"),
    Brick("punct", raw"[-@…–~`'“”’‘|\/$.,:;!?'\u0022]+"),
    Brick("bracket", raw"[\}\{\]\[><\)\(]+"),
