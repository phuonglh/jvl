# phuonglh
# A modified version of GRU to take a 3-d tensor as input. 
# This is useful for batch processing, where each input has shape 
# (d x maxSequenceLength x batchSize).
...
(f::GRU3)(x::Array{Float32,3}) = begin
    ...
    xs = map(i -> sum(f.gru(x[:,:,i]), dims=2), collect(1:m)) # take the bag-of-embeddings (sum)
    hcat(xs...)
end
